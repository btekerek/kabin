...
void main() {
  ...
  test('bootstrap with no stored session resolves to logged-out, not an error', () async {
  ...
  test('login sets state to the logged-in user', () async {
    await container.read(authControllerProvider.future); // settle bootstrap (logged out)

    const testUser = User(id: 1, email: 'guide@example.com', role: 'guide');
    fakeAuthRepository.userToReturn = testUser;
    fakeAuthRepository.loginTokens = const TokenPair(access: 'access-1', refresh: 'refresh-1');
  ...
  test('logout clears state and calls the repository with the stored refresh token', () async {
    await container.read(authControllerProvider.future);
